...
let Popular_Combo_Deals = await getdata(`https://onemg-server-1muy.onrender.com/Popular_Combo_Deals?_page=${page}&_limit=9`)
let Calcium_supplement = await getdata(`https://onemg-server-1muy.onrender.com/Calcium_supplement?_page=${page}&_limit=9`)
let Spotlight = await getdata(`https://onemg-server-1muy.onrender.com/Spotlight?_page=${page}&_limit=9`)
let Treading = await getdata(`https://onemg-server-1muy.onrender.com/Treading?_page=${page}&_limit=9`)
let Breakfast = await getdata(`https://onemg-server-1muy.onrender.com/Breakfast?_page=${page}&_limit=9`)
let Bodymassagers = await getdata(`https://onemg-server-1muy.onrender.com/Bodymassagers?_page=${page}&_limit=9`)
let FootHealth = await getdata(`https://onemg-server-1muy.onrender.com/FootHealth?_page=${page}&_limit=9`)

let AllProducts = await getdata(`https://onemg-server-1muy.onrender.com/ALLProducts?_page=${page}&_limit=9`)
...
function filtereddata(){
    ...
    if(!AllProducts_Filter.checked && !FootHealth_Filter.checked && !Bodymassagers_Filter.checked && !Treading_Filter.checked && !Spotlight_Filter.checked && !Calcium_supplement_Filter.checked && !Popular_Combo_Deal_Filter.checked){
        const randomIndex = Math.floor(Math.random() * (mainarr.length + 1));
        mainarr.splice(randomIndex, 0, ...AllProducts)
    }
    displaydata(mainarr)
...
lowToHigh.addEventListener("change", async ()=>{
    ...
    if(lowToHigh.checked == false && highToLow.checked == false){
        let data = await fetch("https://onemg-server-1muy.onrender.com/ALLProducts?_page=1&_limit=9")
    ...
    let data = await fetch("https://onemg-server-1muy.onrender.com/ALLProducts?_sort=price&_order=asc&_page=1&_limit=9")
    data = await data.json()
...
highToLow.addEventListener("change", async ()=>{
    ...
    let data = await fetch("https://onemg-server-1muy.onrender.com/ALLProducts?_sort=price&_order=desc&_page=1&_limit=9")
    data = await data.json()
    // console.log("this is sorted - " + data)
    displaydata(data)
})
...
document.getElementById("seachbtn").addEventListener("click",async ()=>{
    let search = searchByBrand.value;
     let data = await fetch(`https://onemg-server-1muy.onrender.com/ALLProducts?q=${search}`)
     data = await data.json()
     displaydata(data)
})
